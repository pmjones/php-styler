 ...
 *
 * @phpstan-type styles_array array<token_class_string, style_props_array>
 *
 * @phpstan-type style_props_array array{
 *     spaceBefore?: ?bool,
 *     spaceAfter?: ?bool,
...
class StyleLocator
{
    ...
    public static function fromFormat(Format $format) : self
    {
        $custom = [];

        if ($format->classBracePosition() === 'same_line') {
        ...
        if ($format->functionBracePosition() === 'same_line') {
        ...
        if ($format->controlBracePosition() === 'next_line') {
            foreach (
                [
                    Token\TIfOpeningBrace::class,
                    Token\TElseOpeningBrace::class,
        ...
        if ($format->concatenationSpacing() === false) {
        ...
        if ($format->returnTypeColonSpacing() === false) {
            $custom[Token\TReturnColon::class] = ['spaceBefore' => false];
        }

        if ($format->blankLineAfterBlock() === false) {
